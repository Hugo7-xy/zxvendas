...
export function init(dependencies) {
    console.log("Sistema de Filtro (Abas no Modal) inicializado.");
...
function parsePriceRange(value) { if (value === 'all') { return 'all'; } if (value === '800+') { return { min: 800, max: Infinity }; } const parts = value.split('-'); if (parts.length === 2) { return { min: parseFloat(parts[0]), max: parseFloat(parts[1]) }; } return 'all'; }
